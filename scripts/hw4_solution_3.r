...
# Set the number of cores for parallelization
 # Change this to the number of cores you want to use

cl <- makeCluster(detectCores())  # Initialize a cluster with the specified number of cores
...
clusterExport(cl, c("simTweedieTest", "rtweedie"))
registerDoParallel(cl)  # Register the cluster for parallel processing

# Assignment 2:
MTweedieTests <- function(N, M, sig) {
  result <- foreach(i = 1:M, .combine = 'c') %dopar% {
    sum(replicate(1, simTweedieTest(N)) < sig)
  }
  mean(result) / M
}
